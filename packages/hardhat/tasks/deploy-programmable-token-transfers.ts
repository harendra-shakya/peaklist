...
task(`deploy-programmable-token-transfers`, `Deploys the ProgrammableTokenTransfers smart contract`)
  ...
  .setAction(async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
    ...
    const programmableTokenTransfersFactory: ProgrammableTokenTransfers__factory = await hre.ethers.getContractFactory(
      "ProgrammableTokenTransfers",
    );
    const programmableTokenTransfers: ProgrammableTokenTransfers = await programmableTokenTransfersFactory.deploy(
      routerAddress,
    );
